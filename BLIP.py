...
def load_caption_model(model_name: str):
    ...
    if os.path.isfile(adapter_config_path):
        ...
        return processor, PeftModel.from_pretrained(base_model, model_name)
...
for model_name in model_names:
    processor, model = load_caption_model(model_name)
    file = sys.argv[1]
...
caption_prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a helpful assistant that compares image captions and selects the best one in the context of the image."),
    (
        "user",
        "Captions:\n{captions_list}\n\n"
        "Choose the best caption."
        "Choose exactly one caption from the options. Return only the selected caption as selected_caption in JSON. "
...
result = critic_pipe.invoke({"captions_list": captions_list})

log_caption_selection(
    file,
    captions,
    model_names,
